/**
 * Integer-paise money math — identical algorithm to the backend's
 * src/utils/money.js and the web app's App.jsx, so totals always agree
 * across web, mobile, and server regardless of who computed them first.
 */
...
/** Recomputes an invoice's totals from line items + company/customer state — used for on-screen display and the PDF, mirroring what the backend already validated. */
export function computeInvoice(items, company, customer) {
  ...
  const lineResults = lines.map((it) => {
    const rate = decimalScaled(it.taxRate ?? 0, 2);
    ...
    return { ...it, taxableAmountPaise: it.amountPaise, cgstPaise, sgstPaise, igstPaise, totalPaise: it.amountPaise + taxPaise };
  });
  ...
  return {
    items: lineResults, subtotalPaise: taxablePaise, taxableAmountPaise: taxablePaise,
    cgstPaise: cgstTotalPaise, sgstPaise: sgstTotalPaise, igstPaise: igstTotalPaise,
    roundOffPaise, grandTotalPaise, interstate, companyState, custState,
  };
}
...
function twoDigits(n) { if (n < 20) return ONES[n]; const t = Math.floor(n / 10), r = n % 10; return TENS[t] + (r ? " " + ONES[r] : ""); }
function threeDigits(n) { const h = Math.floor(n / 100), r = n % 100; let s = ""; if (h) s += ONES[h] + " Hundred" + (r ? " " : ""); if (r) s += twoDigits(r); return s; }
